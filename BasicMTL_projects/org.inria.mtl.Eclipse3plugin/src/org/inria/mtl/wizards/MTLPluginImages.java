 ...
 */
public class MTLPluginImages {

	private static final String NAME_PREFIX= "org.inria.mtl."; //$NON-NLS-1$
	...
	// images are used
	static {
		fgIconBaseURL= MTLPlugin.getDefault().getBundle().getEntry("/icons/"); //$NON-NLS-1$
	}
	...
	public static final ImageDescriptor DESC_OBJS_PROJ= createManaged(T_OBJ, IMG_OBJS_PROJ);
	...
	private static ImageDescriptor createManaged(String prefix, String name) {
		try {
			...
			if (fgImageRegistry != null) {
				//MTLPlugin..log("Image registry already defined"); //$NON-NLS-1$
			}
			return result;
		} catch (MalformedURLException e) {
	...
	private static ImageDescriptor createManaged(String prefix, String name, String key) {
		try {
			...
			if (fgImageRegistry != null) {
				//MTLPlugin.logErrorMessage("Image registry already defined"); //$NON-NLS-1$
			}
			return result;
		} catch (MalformedURLException e) {
			return ImageDescriptor.getMissingImageDescriptor();
