$Id: readme.txt,v 1.2 2004-08-31 13:46:01 sdzale Exp $
Description :
This project is an Eclipse plugin for MTL.
It will support:
- automatic build of MTL project (with a nice properties view for fine configuration)
- generation of an ant build file for compilation outside of Eclipse
- syntax highlighting
- code completion
- outline view

History :
---------
$Log: not supported by cvs2svn $
Revision 1.4  2004/06/22 08:39:33  sdzale
Version bêta 1

Revision 1.3  2004/06/15 15:13:19  sdzale
Version assez stable

Revision 1.2  2004/06/03 12:54:27  sdzale
Version 1
MAJ images

Revision 1.1  2004/05/10 13:33:14  dvojtise
initial revision


Liste des variables à créer

ECLIPSE_CORE_RESOURCES
ECLIPSE_UI
ECLIPSE_CORE_RUNTIME
ECLIPSE_CORE_BOOT
ECLIPSE_SWT_WIN32
ECLIPSE_PDE_UI
ECLIPSE_JDT_UI
ECLIPSE_JDT_CORE
APACHE_XERCES
APACHE_XERCES_XMLAPI
ECLIPSE_UI_EDITORS
ECLIPSE_JFACE_TEXT
ECLIPSE_UI_WORKBENCH_TEXTEDITOR
ECLIPSE_JFACE
ECLIPSE_UI_WORKBENCH
ECLIPSE_TEXT
ECLIPSE_UI_VIEWS
ECLIPSE_DEBUG_CORE
ECLIPSE_JDT_LAUNCHING

	

